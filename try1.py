...
class NURBSApp(tk.Tk):
    ...
    def load_model(self, model_name):
        ...
        self.weight_texts = []
        offset = 0.3
        sorted_indices = sorted(range(len(self.ctrlpts)), key=lambda i: self.ctrlpts[i][0])
        for idx, i in enumerate(sorted_indices):
            x, y = self.ctrlpts[i]
            w = self.weights[i]
            y_offset = offset if idx % 2 == 0 else -offset
            t = self.ax.text(x, y + y_offset, f"{w:.1f}", ha='center', color='red', fontsize=10)
            self.weight_texts.append((i, t))
    ...
    def draw_background(self):
        ...
        for (x, y) in self.model_data["tire_coords"]:
            self.ax.add_patch(plt.Circle((x, y), 0.9, fill=True, color='black', zorder=1))

        x0, x1, y = self.model_data["ground_line"]
        self.ax.plot([x0, x1], [y, y], linestyle='-', color='black', linewidth=1, zorder=1)
    ...
    def update_curve(self, event=None):
        ...
        # 重み表示更新
        for i, t in self.weight_texts:
            t.set_position((new_ctrlpts[i][0], new_ctrlpts[i][1] + (0.3 if i % 2 == 0 else -0.3)))
            t.set_text(f"{new_weights[i]:.1f}")
